...
const ResetOrder = require('../app/services/orders/reset/accurate.reset.order')
...
describe('Reset Order', () => {
    it('should return a delete receipt order when Order Receipt exist', () => {
        const order = ResetOrder(orderMock._id)
        chai.expect(order)
    })
    it('should return a delete Invoice order when Order Invoice exist', () => {
        const order = ResetOrder(orderMock._id)
        chai.expect(order)
    })
    it('should return a Reset Order order when Order Accurate exist', () => {
        const order = ResetOrder(orderMock._id)
        chai.expect(order)
    })
})