...
const syncAccurateItem = async (profile_id) => {
    try {
        const item = await itemModel.find({synced: false, upcNo: {$ne: 'rebate'}, profile_id: profile_id, attempts: {$lt: parseInt(maxAttempts)}});
        item.project({accurate_id: 0, _id: 0, synced: 0}).limit(10).toArray(async (err, res) => {
