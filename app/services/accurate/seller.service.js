    ...
    try {
        ...
        if (response.s) {
            ...
            if (db_session === seller.api_db_session) {
                ...
                await sellerModel.update({ seller_id: seller.seller_id }, { $set: { api_db_session: db_session } });
            }
        } else {
...
const getOpenDB = async (seller) => {
    try {
        ...
        if (response.s) {
            ...
            await sellerModel.update({ seller_id: seller.seller_id }, { $set: { open_db_id: db_id } });
            return db_id;
        } else {
...
const refreshTokenService = async (seller) => {
    try {
        ...
        if (response.access_token) {
            await sellerModel.update({ seller_id: seller.seller_id }, { $set: { api_access_token: response.access_token } });
        } else {
            await helper.errLog(seller.seller_id, payload, response, 1);
        }
    } catch (error) {
